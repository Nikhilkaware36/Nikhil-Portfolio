      ...
      <div className="container mx-auto px-4">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {[
              { icon: Target, title: "Dark Web Intel", desc: "Deep dive investigations and threat intelligence gathering from hidden networks.", color: "text-neon-green", border: "hover:border-neon-green/50" },
              { icon: Globe, title: "OSINT Research", desc: "Open-source intelligence automation and large-scale data analysis.", color: "text-electric-purple", border: "hover:border-electric-purple/50" },
              { icon: Bug, title: "Bug Bounty", desc: "Vulnerability hunting and responsible disclosure on live targets.", color: "text-signal-red", border: "hover:border-signal-red/50" },
            ].map((item, index) => (
              <motion.div
                key={item.title}
                initial={{ opacity: 0, y: 30 }}
                ...
                className={`p-6 bg-card border border-border/50 rounded-lg ${item.border} transition-colors group`}
              >
                <item.icon className={`w-8 h-8 ${item.color} mb-4 group-hover:animate-pulse`} />
